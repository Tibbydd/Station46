...
var recoil_return_velocity: float = 0.0
...
func _update_hud() -> void:
	if not ammo_label:
		return
	var glasses_online := has_wearable_module("hud_glasses") and not glasses_power_empty
	...
	if body_label.visible:
		...
		if has_wearable_module("sound_meter"):
			var meter := _get_estimated_noise_readout()
			body_text += "SOUND %s\n" % meter
		if glasses_power_load > 3.0:
			body_text += "FRAME LOAD %.1fU\n" % glasses_power_load
		if glasses_lens_damage > 0.15:
			body_text += "LENS DAMAGE %.0f%%\n" % (glasses_lens_damage * 100.0)
	...
	if treatment_label.visible:
		if health.active_treatment.is_empty():
			...
			var anchor_text := "   X TAP/HOLD"
			if neural_anchor_active:
				anchor_text = "   X HOLD %.0f%%" % (neural_anchor_time / neural_anchor_required * 100.0)
			var carry_text := "   CARRYING" if carried_object else ""
			treatment_label.text = "E USE   F SHOVE/THROW   H MAG CHECK   I INSPECT   B BANDAGE   T TRAUMA" + anchor_text + build_text + foul_text + stealth_text + carry_text
		else:
...
func equip_found_weapon(weapon_id: String) -> void:
	if weapon:
		if weapon.data and not weapon_dropped:
			_spawn_weapon_pickup_from_state(weapon.get_weapon_state(), global_position + -camera.global_transform.basis.z * 0.65 + Vector3.UP * 0.25)
...
func _install_wearable_module_internal(module_id: String, show_conflict: bool) -> bool:
	...
	if slot == "glasses":
		...
		var module_slots: Array = wearable_slots.get(slot, [])
...
func _get_inventory_summary() -> String:
	...
	return "LOADOUT: %s / %s / %s / %s / SUP %d%s" % [background.to_upper(), weapon_text, light_text, comms_text, resource_count, route_noise]
...
func can_operate_weapon() -> bool:
	return not weapon_dropped and not carried_object and health.can_hold_weapon()
...
func _spawn_weapon_pickup_from_state(state: Dictionary, position: Vector3) -> EquipmentPickup3D:
...
func _apply_background_traits(background: String) -> void:
	...
	if traits.has("resource_bonus"):
		var current_resources: Dictionary = survivor_loadout.get("resources", {})
...
func _on_weapon_recoil_requested(amount: float) -> void:
	var recoil := deg_to_rad(amount * recoil_trait_modifier)
	pitch = clamp(pitch + recoil, deg_to_rad(-82.0), deg_to_rad(82.0))
	head.rotation.x = pitch
	recoil_return_velocity += recoil * 0.35
